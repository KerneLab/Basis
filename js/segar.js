 ...
 * center; }
 * 
 * .segar-txt-hint { color: #333333; display: none; float: left; height: 20px;
 * left: 0px; line-height: 20px; position: absolute; text-align: center; top:
 * 0px; width: 200px; z-index: 100; }
 */
(function($) {
	$.fn.segar = function(options) {
		...
		var darkText = $("<div></div>").addClass(opts["txt.hint.class"]).text(
		...
		this.to = function(rates) {
			...
			d3.select(node).selectAll("." + opts["txt.hint.class"]) //
			.transition().duration(duration).tween("text", function(d, i, a) {
